...
```
```{sql connection=dbcon}
dbWriteTable(dbcon, "pub_med_pub_date", PubMedPubDate.df, overwrite = F, row.names = FALSE, append=TRUE)
...
#AuthorRecord.df$AuthorListID <- TempAuthorShip.df$ArticleID

aLists <- TempAuthorShip.df$ArticleID
...
for (i in 1:numPA) {
    ...
    #now lg is language node
     ...
     # now retrieve the ID fk from other tables
